    ...
    useEffect(() => {
        const fetchParcels = async () => {
            try {
                ...
                const processedParcels = parcelsData.filter((parcel) => parcel.status?.trim() === "На складе");
    ...
    return (
        <Container>
            ...
            <MainContent>
                <Sidebar />
                <Content>
                    <GreenHeading>Принятые посылки</GreenHeading>
                    <Divider />
                    <Form>
                        <InputContainer>
                            <SearchIconContainer>
                    ...
                    <DataGrid>
                        {filteredParcels.map((parcel, index) => (
                            <React.Fragment key={index}>
                                <DataCell style={{ gridColumn: "span 1" }}>{parcel.shelf || "Еще не обработана"}</DataCell>
                                ...
                                <DataCell style={{ gridColumn: "span 2" }}>
                                    <PrintIcon src={FiPrinter} alt="Print" onClick={() => printParcel(parcel)} />
                                </DataCell>
                            </React.Fragment>
                        ))}
                    </DataGrid>
