...
const Otgoingparcels = () => {
    ...
    useEffect(() => {
        const fetchParcels = async () => {
            try {
                ...
                const processedParcels = parcelsData.filter((parcel) => parcel.status?.trim().toLowerCase() === "исходящая");
    ...
    const printParcel = (parcel) => {
        ...
        const trackingNumber = parcel.trackingNumber || "N/A"; // Убедитесь, что трек-номер определен
    ...
    return (
        <Container>
            ...
            <MainContent>
                <Sidebar />
                <Content>
                    <GreenHeading>Обработанные посылки</GreenHeading>
                    <Divider />
                    <Form>
                        <InputContainer>
                            <SearchIconContainer>
